...
```
- `node_item_name_confirm`：改写问题 + 提取 item_names / 资料线索 → 匹配真实主体 → 四种去向：
  ① 有可信主体（≥0.70）→ 确认并继续检索；
  ② 无可信但有候选（0.60~0.70）且候选与问题字面相关 → 写 `answer` 反问用户选择；
  ③ **指向不明 → 反问澄清**：用**确定性规则**判定（`_need_clarify`：指示代词+对象名词 / 「某X」，且未抽出主体），反问时列出知识库可选资料；
  ④ 其余 → 清空 item_names 走检索；若问题点名了某份资料（`file_hints` 命中且不过泛）则写入 `doc_filters`，检索按 `file_title` 收窄（需求 §11.4）
- 多路召回：普通向量 + HyDE；**联网搜索默认关闭**（`state['disable_web_search']` 默认 `True`）；普通向量那路的过滤优先级为 **主体 > 资料名(file_title) > 无过滤**，且过滤后召回不足 3 条会自动去掉过滤重试（HyDE 那路不加过滤，天然留一路全库兜底）
- `node_answer_output`：拼 `answer_out.prompt`（结构化输出 + 合规护栏 + 无资料兜底 + 禁止用资料外常识补全）→ LLM（支持 SSE 流式 delta）→ 抽取图片 URL → 构建 `references` 引用来源 → 存历史
...
8. **需求 §11 问法补强（2026-09-22 下午）**：① **指向不明 → 反问澄清**（`_need_clarify` 确定性规则：指示代词+对象名词 / 「某X」，且未抽出主体；反问时列出知识库可选资料）；② **文档级过滤**（问题点名资料时抽 `file_hints` → 匹配 `file_title` 白名单 → 检索收窄，见 `doc_filters`）；③ **反编造禁令**（`answer_out.prompt` 禁止用资料外常识补全定义/分级标准，如资料没写 R3 就不许解释 R3）；④ 兜底边界明确为「能支撑主要部分就作答并说明缺口，否则原样兜底」。
9. **模型单例并发修复**：`get_bge_m3_ef()` / `get_reranker_model()` 加**双重检查锁**。此前两个并行检索分支会同时首次加载同一模型，导致其中一个实例权重停在 PyTorch meta 设备上，encode 报 `Cannot copy out of meta tensor`（实测 10/10 题全挂；已复现并修复）。
...
6. **规则断言的边界**：`must_have`/`must_not` 仍是字面匹配 —— 数字已支持千分位与单位换算、`must_not` 已支持否定式排除（8 字窗口）与**引号内提及**（「任何关于"保证收益"的说法均不符合事实」不算违规）、兜底判定已改为「整体兜底」（答完正文、末尾仅补一句无资料提示不算兜底）；但同义改写仍可能漏判，带语义的合规判断以合规 rubric 的 LLM 评分为准。
...
8. **「作答 vs 兜底」边界存在运行间波动**：当资料只覆盖问题的一半时（如 `risk-08` 问提前赎回/提前终止，资料只有提前终止），模型时而作答并说明缺口、时而整段兜底。属 LLM 固有方差，复跑同题结果可能不同。
9. **题库两处口径待定**：`ux-08`/`ux-09` 是泛化问题，答案可能来自同类任一份资料（实测分别来自中银理财揭示书、基金基础知识），而题库里标的是单一份 `gold_sources`，导致检索命中显示 False；`ux-09` 还缺关键词「开放日」。
...
**v5（同批 43 题，判官 qwen-flash，2026-09-22 傍晚）**：标准来源命中率 41/41｜recall 0.984｜MRR 1.0｜忠实度 0.697｜答案相关性 0.722｜上下文精确率/召回率 0.872/0.901｜事实正确性 0.587｜合规 0.988｜无违规词 43/43｜关键词齐全 36/43｜兜底判断 42/43｜**反问判断 43/43**｜references 非空 43/43。
另：题库新增 11 道需求 §11 题（`ux-01`~`ux-11`），**行为正确 11/11**（4 题正确反问、2 题正确兜底、5 题正常作答，其中 2 题验证文档过滤链路）。
- v5 相对 v2 的取舍：无结构性回退（命中率、引用、合规持平，事实正确性略升）；忠实度降 0.07（疑为判官波动 + 多轮提示词调整所致，需复跑确认）；关键词齐全 36/43（`fund-06`/`report-05`/`risk-02` 为断言措辞与表格类老问题）。
